...
def check():  # This will insert what is in the code_attempt Entry
    ...
    if code_attempt == code:
        results_listbox.insert(
            tk.END, "{} is CORRECT! YOU WIN!".format(code_attempt))
        results_listbox.see(tk.END)
        entry_code_attempt.delete(0, tk.END)
        ...
        # Add results to listbox and clear Entry field
        results_listbox.insert(
            tk.END, code_attempt_for_listbox+'   '+results_for_listbox)
        entry_code_attempt.delete(0, tk.END)
        results_listbox.see(tk.END)
...
# This button goes in next to the code entry and
# will 'get' the information from the entry widget and
# executes the 'check_int' and then the 'check' function.
b = tk.Button(window, text="Check!", width=15, command=check_int)
b.grid(row=1, column=1, padx="60")
window.bind('<Return>', lambda event: check_int())
...
results_listbox = tk.Listbox(
    window, width=50, xscrollcommand=scrollbar.set)
